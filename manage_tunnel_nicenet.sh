...
# 
# Purpose:
#   - IRAN:   migrate udp -> backhaul, rollback backhaul -> udp, remigrate udp -> backhaul
#   - KHAREJ: apply tasks file to create backhaul clients that connect to IRAN (matching WEB_PORT)
#
# Notes:
#   - Does NOT delete udp services. Only stop+disable during migration.
#   - Dedup rule: if multiple udp units bind the same local port (-l ...:<LPORT>), keep the newest unit file.
...
discover_udp_active_dedup() {
  cecho "Discovering ACTIVE udp*.service units..."
  local units=()
  mapfile -t units < <(systemctl list-units --type=service 'udp*.service' --no-legend --no-pager 2>/dev/null | awk '{print $1}' || true)

  if [ "${#units[@]}" -eq 0 ]; then
    yecho "No udp*.service units found."
    return 0
  fi
  ...
  cecho "Selected ${#SELECTED_UNITS[@]} unique ACTIVE udp unit(s) after dedup."
}
...
mode_migrate_iran() {
  ...
  for lport in $(printf "%s\n" "${!SELECTED_UNITS[@]}" | sort -n); do
    ...
    # Requested order: stop+disable udp first, then start backhaul
    ...
    OK+=("lport=$lport udp=$unit -> backhaul=$svc_unit web_port=$web_port inter_port=$inter_port foreign=$foreign_ip")
...
mode_rollback_iran() {
  ...
  cecho "Rolling back: backhaul -> udp (IRAN)"
  systemd_reload
  ...
  for unit in "${units[@]}"; do
    ...
    udp_unit="udp${lport}.service"
    cecho "---- ROLLBACK lport=${lport} backhaul=${unit} -> udp=${udp_unit} ----"

    svc_stop_disable "$unit"

    if svc_exists "$udp_unit"; then
      if ! svc_enable_start "$udp_unit"; then
        FAIL+=("lport=$lport backhaul=$unit udp=$udp_unit reason=Udp2rawStartFailed")
        print_last_logs "$udp_unit"
        continue
      fi
      OK+=("lport=$lport backhaul=$unit -> udp=$udp_unit")
...
mode_remigrate_iran() {
  ...
  discover_udp_active_dedup
  if [ "${#SELECTED_UNITS[@]}" -eq 0 ]; then
    yecho "No ACTIVE udp units to re-migrate."
    return 0
  fi
  ...
  for lport in $(printf "%s\n" "${!SELECTED_UNITS[@]}" | sort -n); do
    ...
    cecho "---- RE-MIGRATE lport=${lport} udp=${unit} -> ${svc_unit} ----"
    ...
    OK+=("lport=$lport udp=$unit -> backhaul=$svc_unit")
...
prompt_mode_if_needed() {
  ...
  if [ "$ROLE" = "iran" ]; then
    ...
    printf "  [1] Migration  udp -> backhaul\n  [2] Rollback   backhaul -> udp\n  [3] Re-Migrate udp -> backhaul (only if backhaul already exists)\n" >&2
